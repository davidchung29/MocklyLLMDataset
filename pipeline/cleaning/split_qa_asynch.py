...
async def is_question_api(text:str)->bool:
 async with semaphore:
  messages=[{"role":"system","content":("You are a data cleaner. Your only job is to categorize whether a sentence is a question. "
...
async def fix_punctuation(text:str)->str:
 ...
 if not text:return ""
 if text[-1] not in ".!?":
  if await is_question_api(text):text+="?"
  else:text+="."
...
async def split_into_qa(text):
 ...
 while i<len(sentences):
  if await is_question_api(sentences[i]):break
  i+=1
 while i<len(sentences):
  sent=sentences[i].strip()
  if await is_question_api(sent):
   if question is not None and not answer_parts:question+=" "+sent
   else:
...
async def main():
 ...
 try:limit=int(limit)
  ...
  print("invalid input")
  limit=0
 if limit==-1 or limit>len(all_files):limit=len(all_files)
 tasks=[]
